...
if (navigator.geolocation) {
  ...
  navigator.geolocation.getCurrentPosition(getWeather, (err) => {
    ...
    if (err.message.startsWith("Only secure origins are allowed")) {
      errMessage =
        "getCurrentPosition API only works on secure (HTTPS) domains";
    }
    error.classList.add("show");
    error.textContent =
      errMessage + ". Will fall back to showing weather for London";
  ...
  error.classList.add("show");
  error.textContent =
    "Geolocation is not supported by this browser. Maybe it's time to upgrade!";
...
async function getWeather(pos) {
  ...
  try {
    ...
    if (!resp.ok)
      throw `Fetch /api/weather/${lat}/${long} failed with ${resp.statusText}`;
    const data = await resp.json();

    weatherIcon.classList.add(`owi-${data.weather[0].icon}`);
    locationSpan.textContent = ` - ${data.name}`;
    addWeatherDetails(
      `The weather currently is: &nbsp; ${data.weather[0].description}`,
    );
    ...
    addWeatherDetails(
      `Volume of rain in last hour: &nbsp; ${data.rain ? data.rain["1h"] : 0}mm`,
    );
    addWeatherDetails(
      `Wind speed: &nbsp; ${(data.wind.speed * 2.24).toFixed(1)} mph`,
    );
  } catch (err) {
    error.classList.add("show");
